...
var minimumTime = function (n, edges, disappear) {
  ...
  for (let [from, to, length] of edges) {
    graph[from].push([to, length]);
    graph[to].push([from, length]);
  }

  let minTime = new Array(n).fill(-1);
  let pq = new MyPriorityQueue((a, b) => a[1] - b[1]);
  pq.enqueue([0, 0]);
  let visited = new Set();
  while (pq.isEmpty() == false) {
    let [currentNode, currentTime] = pq.dequeue();
    if (visited.has(currentNode)) continue;
    if (disappear[currentNode] > currentTime) {
      minTime[currentNode] = currentTime;
    } else {
      minTime[currentNode] = -1;
      continue;
    }
    visited.add(currentNode);
    let adj = graph[currentNode];
    for (let [nextNode, time] of adj) {
      pq.enqueue([nextNode, time + currentTime]);
    }
  }
  return minTime;
};
...
console.log(minimumTime(n, edges, disappear));
